...
class OrderOpenPosition(models.Model):
    ...
    initial_quantity = models.FloatField(blank=False,
                                         validators=[MinValueValidator(0.000001, message="Enter a positive number")])
    ...
    leverage = models.IntegerField(default=1, blank=True,
                                   validators=[MinValueValidator(0, message="Enter a positive number")])
    ...
    def __str__(self):
        return "ID=" + str(
            self.id) + " --- " + self.symbol.symbol + "---direction= " + self.get_direction_display() + "---initial quantity= " + str(
            self.initial_quantity) + "---current quantity= " + str(self.current_quantity) + "---open price= " + str(
            self.matched_price) + '--blocked margin=' + str(self.blocked_margin) + '--unrealized gain=' + str(
    ...
    def unrealized_gain(self):
        if self.is_an_open_position:
            if self.direction == OrderOpenPosition.LONG:
                return profit_or_loss_calculator(self.symbol.bid, self.matched_price, self.current_quantity,
                                                 self.symbol, self.direction)
            else:
                return profit_or_loss_calculator(self.symbol.ask, self.matched_price, self.current_quantity,
                                                 self.symbol, self.direction)
        else:
            return 0
...
class OrderClosePosition(models.Model):
    ...
    def __str__(self):
        return "Close Order for: " + str(
            self.related_open_order.symbol) + "---" + self.related_open_order.get_direction_display() + '---Close price: ' + str(
            self.close_position_price) + " closed position " + str(
            self.quantity) + " Result= " + self.get_result_display()

    @property
    def realized_gain(self):
        if self.result == 'S':
            return profit_or_loss_calculator(self.close_position_price, self.related_open_order.matched_price,
                                             self.quantity, self.related_open_order.symbol,
                                             self.related_open_order.direction)
        else:
            return 0
